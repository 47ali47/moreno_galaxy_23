...
def handle_ring(ring_criteria, all_stars):
    ...
    Then return the number of stars in the ring and their total mass.
    :param ring_criteria: (inner_radius, outer_radius)
    :param all_stars: [((x, y), mass)...((x, y)), mass)]
    :return: (number_of_stars_in_the_ring, total_star_mass_in_the_ring)
    """
    ...
    return (star_count, total_star_mass)

#all_stars is a list of the stars coordinate and mass
#[((x, y), mass)...((x, y)), mass)]
all_stars = []

rings = {} #{ring_index:(star_count, total_star_mass_in_ring)}
...
#for (ring_index, (star_count, total_star_mass)) in rings.items():
    #print('ring index', ring_index)
    #print('star_count', star_count)
    #print('total_star_mass', total_star_mass)


#plot for rings and total_mass/star_count
...
y_values = [mass for (count, mass) in rings.values()]
...
x_values = rings.keys() #the ring indexes e.g. 0, 1, 2, 3...
y_values = [count for (count, mass) in rings.values()]
...
ring_star_mass = [mass for (count, mass) in rings.values()]
...
plot_star_rings(filename = 'accumulated_star_mass_plot.pdf',
                title = 'Scatter Plot of Star Radius (in rings) vs. Accumulated Mass',
                x_label = 'Rings',
